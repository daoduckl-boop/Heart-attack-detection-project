...
Hadheartattack,Hadangina,hadstroke,haddiabetes and chestscan are binary 
...
* My comment: the heart attack data is immbalance 

Analyze the relationships between target and other variables
```{r}
# Analysis by gender
...
My comment: 
- The percentage of people smolking every day is highest
...
In addition, we explore the optional use of Principal Component Analysis (PCA) to reduce dimensionality while preserving most of the dataset's variance. This can help improve model performance, particularly in algorithms sensitive to feature redundancy.
...
Variables with little to no variation (e.g., same value for most observations) are uninformative for the model and can be safely dropped. The nearZeroVar() function from the caret package detects such variables. In our case, no features were removed due to near-zero variance. Essentially, all the features in our dataset have sufficient variability, and none were considered redundant based on their variance.
```{r}
...
# Check data distribution before processing
```{R}
...
```
# 2. APPLY DOWNSAMPLING TO TRAINING DATA ONLY
```{r}
...
# Combine to create balanced training dataset
...
# Convert target to numeric (0/1) for logistic regression
```{r}
...
```
# Create model formula with all available  predictors
```{R}
...
print(paste("\nTotal number of predictors:", length(predictor_vars)))
```
...
# 5. EVALUATE MODEL ON ORIGINAL TEST DATA
...
```
# Performance Analysis of Random Forest Model in Heart Attack Detection Project
